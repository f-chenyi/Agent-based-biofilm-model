//
// Created by FeiChenyi on 5/27/18.
//

#ifndef BIOFILM_AGENT_AGENT_V1_H
#define BIOFILM_AGENT_AGENT_V1_H
...
const double A_1 = E_1 * R / 10000.0; 	// cell-surface adhesion energy (cylindrical term)
...
double Lf = 6 * R; // mean final length
...
const double dtTHRESH = h0*1e-8; //

// Time evolution parameters and output:
const double T = 5; // Integrate for this amount of total time.
...
my6Vec cell_cell_gforce(int i, int j) {
    ...
    // *** Comment: lacks a R^(1/2) term in the second equation?
    double QT = (2*A_0)/sqrt(qij2) - 5*E_0*pow(R,0.5)*pow(-pow(qij2,0.16666666666666666) +
...
#endif //BIOFILM_AGENT_AGENT_V1_H
